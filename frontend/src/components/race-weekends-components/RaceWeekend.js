...
export default function RaceWeekend(){
    const { selectedRaceWeekend, selectedRaceWeekendLoading } = useRaceWeekendContext();
    ...
    return(
        <div className="card rounded-15 element-background-color element-border-color">
            <div className="card-header rounded-15 clickable nested-element-color m-2">
                <div className="d-flex align-items-center p-1">
                    ...
                    <div className="ms-auto">
                        <div className="container">
                            {selectedRaceWeekend.race.status == 2 && <span className="badge rounded-pill text-bg-success">Final</span>}
                            {selectedRaceWeekend.race.status == 1 && <span className="badge rounded-pill text-bg-warning">In progress</span>}
                            {selectedRaceWeekend.race.status == 0 && <span className="badge rounded-pill text-bg-secondary">Upcoming</span>}
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div className="card rounded-15 element-background-color element-border-color mb-2" id="race-result-card">
                        ...
                        <div ref={raceDivRef} className="card-body custom-card-body" id="race-positions-card-body">
                            <div className="rounded-15 nested-element-color p-1">
                                ...
                                {selectedRaceWeekend.finalized && selectedRaceWeekend.competitors_positions.map((competitor_position) => (
                                ...
                                {!selectedRaceWeekend.finalized && selectedRaceWeekend.qualifying_positions.map((qualifying_position) => (
            ...
            {(selectedRaceWeekend.finalized && selectedRaceWeekend.standings.users_picks.length > 0) && 
