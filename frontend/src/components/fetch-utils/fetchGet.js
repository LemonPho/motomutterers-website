...
export async function getUserPicks(seasonId, username){
    let response = {
        error: false,
        userPicks: null,
        status: null,
    ...
    try{
        const apiResponse = await fetch(`/api/get-user-picks?season=${seasonId}&user=${username}`);
        const apiResult = apiResponse.status === 200 ? await apiResponse.json() : false;
...
export async function getUserPicksSimple(seasonYear, username){
    let response = {
        error: false,
        userPicks:null,
        status: null,
    ...
    try{
        const apiResponse = await fetch(`/api/get-user-picks-simple?season=${seasonYear}&username=${username}`);
        const apiResult = apiResponse.status === 200 ? await apiResponse.json() : false;
