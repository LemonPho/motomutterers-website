...
export default function Textarea({ id, value, setValue, onEnterFunction, placeholder, className }){
    ...
    return(
        <textarea className={`input-field w-100 textarea-expand ${className}`} id={id} ref={textareaRef} rows={1} defaultValue={value} placeholder={placeholder} onChange={(e) => {keyPress(e)}}></textarea>
    );
}